...
#[pymethods]
impl PyDiagnosticStyledNote {
    /// Construct a non-empty styled diagnostic note.
    #[requires(true)]
    #[ensures(ret.is_ok() || ret.is_err())]
    #[new]
    fn new(
    ...
    /// Return when the note should be displayed.
    #[requires(true)]
    #[ensures(true)]
    #[getter]
    fn mode(&self, py: Python<'_>) -> PyResult<Py<PyAny>> {
    ...
    /// Return the immutable styled note segments.
    #[requires(true)]
    #[ensures(true)]
    #[getter]
    fn segments(&self, py: Python<'_>) -> PyResult<Py<PyTuple>> {
...
impl PyTraceReport {
    ...
    /// Return the immutable trace event sequence.
    #[requires(true)]
    #[ensures(true)]
    #[getter]
    fn events(&self, py: Python<'_>) -> PyResult<Py<PyTuple>> {
    ...
    /// Report whether the trace event limit truncated this report.
    #[requires(true)]
    #[ensures(ret == self.value.truncated)]
    #[getter]
    fn truncated(&self) -> bool {
    ...
    /// Return the optional structured failure summary.
    #[requires(true)]
    #[ensures(true)]
    #[getter]
    fn failure(&self) -> Option<PyTraceFailureSummary> {
...
#[pymethods]
impl PyTraceContext {
    /// Construct a non-empty trace context over an ordered byte range.
    #[requires(true)]
    #[ensures(ret.is_ok() || ret.is_err())]
    #[new]
    fn new(construct: String, byte_start: usize, byte_end: usize) -> PyResult<Self> {
    ...
    /// Return the active grammar construct name.
    #[requires(true)]
    #[ensures(ret == self.value.construct.as_str())]
    #[getter]
    fn construct(&self) -> &str {
