...
pub mod generated_model {
    ...
    // the Zantufa `mex` language also contains the two baseline quantifier
    // surfaces, so it would otherwise steal ordinary CLL quantifiers and
    // mark them experimental. The refinement rejects a completed `mex` that is
    // exactly one of those surfaces, and strict ordered choice then reparses it
    // through `mekso_quantifier` or `pa_run_quantifier`.
    //
