...
pub(crate) use type_invariant::{invariant_enum, invariant_struct};
...
impl ContractType {
    ...
    pub(crate) fn contract_type_and_mode(ident: &str) -> Option<(ContractType, ContractMode)> {
        match ident {
            ...
            "expensive_requires" => Some((ContractType::Requires, ContractMode::Expensive)),
            "expensive_ensures" => Some((ContractType::Ensures, ContractMode::Expensive)),
            "expensive_invariant" => Some((ContractType::Invariant, ContractMode::Expensive)),
            _ => None,
        }
    }
}
