...
fn legacy_as_generated_flat_or_composite_tense_words_tree_value(
    ...
    if let Some(value) = legacy_as_generated_time_tense_sequence_tree_value(&tokens, source, options)
    {
        return Some(value);
    }
    if let [ki] = words
...
fn legacy_as_generated_flat_modal_tense_tree_value(
    ...
    let se = words
        .get(index)
        .filter(|word| word.is_selmaho(Selmaho::Se));
    if se.is_some() {
        index += 1;
    }
    let bai = words
...
fn legacy_as_generated_simple_term_tree_value(
    ...
    match term.as_data() {
        ...
        bityzba::data!(jbotci_syntax::ast::TermSyntax::JaiTaggedSumti { jai, tag, sumti }) => {
            let mut entries = vec![TreeEntry {
                label: Some("jai"),
                value: required_legacy_syntax_subtree_value(jai, source, options),
            }];
            if let Some(tag) = tag {
                entries.push(TreeEntry {
                    label: Some("tag"),
        ...
        bityzba::data!(jbotci_syntax::ast::TermSyntax::Termset {
            ...
            if let Some(nuhu) = nuhu {
                entries.extend(legacy_token_field_entries(closing_label, nuhu, source, options));
            }
            TreeValue::Node(TreeNode {
                constructor,
                entries,
        ...
        bityzba::data!(
            ...
            if let Some(m_nuhi) = m_nuhi {
                entries.extend(legacy_token_field_entries("m_nuhi", m_nuhi, source, options));
            }
            entries.push(TreeEntry {
                label: Some("gek"),
                value: legacy_as_generated_connective_tree_value(gek, source, options),
...
fn legacy_as_generated_reverse_polish_parts_tree_value(
    ...
    let (first_operand, remaining_operands) = operands.split_first()?;
    if remaining_operands.len() != operators.len() {
        return None;
    }

    let mut entries = vec![TreeEntry {
        label: Some("first_operand"),
        value: legacy_as_generated_mekso_operand_tree_value(first_operand, source, options),
    }];
    if let Some((last_operator, nested_operators)) = operators.split_last() {
        let nested_parts = legacy_as_generated_reverse_polish_parts_tree_value(
            remaining_operands,
            nested_operators,
            source,
            options,
        ...
        entries.push(TreeEntry {
            ...
            value: TreeValue::Collection(vec![TreeValue::Collection(vec![
                nested_parts,
                legacy_as_generated_mekso_operator_tree_value(last_operator, source, options),
            ])]),
        });
    }
...
fn legacy_as_generated_simple_mekso_operand_tree_value(
    ...
    match mekso.as_data() {
        ...
        bityzba::data!(jbotci_syntax::ast::MeksoSyntax::ForethoughtMeksoConnection {
            gek,
            left_expression,
            gik,
            right_expression,
        ...
        match self.as_data() {
            ...
            bityzba::data!(jbotci_syntax::ast::TanruUnitSyntax::TagSelbri { xohi, tag }) => {
                TreeValue::Node(TreeNode {
                    ...
                    entries: vec![
                        TreeEntry {
                            label: Some("xohi"),
                            value: required_legacy_syntax_subtree_value(xohi, source, options),
                        },
                        TreeEntry {
                            label: Some("tag"),
                            value: legacy_as_generated_tense_modal_tree_value(
            ...
            bityzba::data!(jbotci_syntax::ast::TanruUnitSyntax::OrdinalSelbri { number, moi }) => {
                TreeValue::Node(TreeNode {
                    ...
                    entries: vec![
                        ...
                        TreeEntry {
                            label: Some("moi"),
                            value: required_legacy_syntax_subtree_value(moi, source, options),
                        },
                    ],
                })
            }
            ...
            bityzba::data!(jbotci_syntax::ast::TanruUnitSyntax::TextSelbri {
                ...
                let mut entries = vec![
                    TreeEntry {
                        label: Some("luhei"),
                        value: required_legacy_syntax_subtree_value(luhei, source, options),
                    },
                    TreeEntry {
                        label: Some("text"),
                        value: legacy_as_generated_text_tree_value(text.as_ref(), source, options),
                ...
                if let Some(liau) = liau {
                    entries.push(TreeEntry {
                        label: Some("liau"),
                        value: required_legacy_syntax_subtree_value(liau, source, options),
                    });
                }
                TreeValue::Node(TreeNode {
...
fn legacy_as_generated_linked_sumti_tree_value(
    ...
    match (fa, sumti) {
        ...
        (None, Some(sumti)) => {
            ...
            if let Some((tense_modal, inner_sumti)) =
                legacy_tense_tagged_linked_sumti_parts(sumti)
            {
                return TreeValue::Node(TreeNode {
...
fn legacy_place_tagged_linked_sumti_parts(
    ...
    match sumti.as_data() {
        bityzba::data!(jbotci_syntax::ast::SumtiSyntax::TaggedSumti {
            tag,
            inner_sumti,
        }) => match tag.as_data() {
...
fn legacy_tense_tagged_linked_sumti_parts(
    sumti: &jbotci_syntax::ast::SumtiSyntax,
) -> Option<(&jbotci_syntax::ast::TenseModalSyntax, &jbotci_syntax::ast::SumtiSyntax)> {
    match sumti.as_data() {
        bityzba::data!(jbotci_syntax::ast::SumtiSyntax::TaggedSumti {
            tag,
            inner_sumti,
...
fn legacy_as_generated_connected_tense_modal_tree_value(
    ...
    while index < tokens.len() {
        let connective = tokens[index];
        if !legacy_token_is_tag_connective(connective) {
            return None;
        }
        index += 1;
        ...
        continuations.push(TreeValue::Collection(vec![
            generated_token_tree_value(connective, source, options),
            atom,
        ]));
    }
...
fn legacy_as_generated_connected_tense_atom_tree_value(
    ...
    *index = start;
    ...
    let end = tokens[*index + 1..]
        .iter()
        .position(|token| legacy_token_is_tag_connective(token))
        .map_or(tokens.len(), |relative| *index + 1 + relative);
    if end == *index {
        return None;
    }
    ...
    let value = legacy_as_generated_time_tense_sequence_tree_value(
        &tokens[*index..end],
        source,
        options,
    )?;
...
fn legacy_as_generated_flat_tense_modal_atom_tree_value(
    ...
    *index += 1;
    ...
    let ki = tokens
        .get(*index)
        .filter(|token| token.is_cmavo(Cmavo::Ki));
    if ki.is_some() {
        *index += 1;
    }
    if *index == start {
...
fn legacy_as_generated_time_tense_sequence_tree_value(
    ...
    loop {
        ...
        if tokens[index].is_selmaho(Selmaho::Zaho) {
            ...
            let roi = tokens.get(index).filter(|token| token.is_selmaho(Selmaho::Roi))?;
            index += 1;
            let mut entries = vec![
                TreeEntry {
                    label: Some("number"),
    ...
    let space = legacy_as_generated_space_tense_sequence_tree_value(tokens, &mut index, source, options)?;
    let caha = if tokens
        .get(index)
...
fn legacy_as_generated_space_tense_sequence_tree_value(
    ...
    let interval =
        legacy_as_generated_space_interval_tense_value(tokens, index, source, options)?;
    let mohi = if tokens
        .get(*index)
...
fn legacy_as_generated_fehe_interval_property_tense_value(
    ...
    let mut next = *index;
    let fehe = tokens.get(next).filter(|token| token.is_cmavo(Cmavo::Fehe))?;
    next += 1;
    let property =
...
fn legacy_as_generated_interval_property_tense_value(
        ...
        .is_some_and(|token| token.is_selmaho(Selmaho::Pa))
    {
        ...
        let roi = tokens.get(next).filter(|token| token.is_selmaho(Selmaho::Roi))?;
        next += 1;
        let mut entries = vec![
            TreeEntry {
                label: Some("number"),
...
#[requires(true)]
#[ensures(true)]
fn legacy_token_is_tag_connective(token: &Token) -> bool {
    token.is_selmaho(Selmaho::Ja)
        || token.is_selmaho(Selmaho::Joi)
        || token.is_selmaho(Selmaho::Bihi)
        || token.is_selmaho(Selmaho::Gaho)
}
