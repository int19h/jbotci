...
fn postfix_parser_output_type(
    ...
    match (method.to_string().as_str(), args.len()) {
        ...
        // Output refinement only decides whether a match survives; an accepted
        // match keeps the inner parser's output type.
...
fn elidable_terminator_terminal_cmavo(expr: &Expr) -> Option<String> {
    match expr {
        ...
        Expr::MethodCall(method) => match (method.method.to_string().as_str(), method.args.len()) {
            ...
            | ("warn", 1)
            | ("reject_output", 1) => elidable_terminator_terminal_cmavo(&method.receiver),
            _ => None,
        },
...
fn classify_postfix_recovery_expr(
    ...
    match (method.to_string().as_str(), args.len()) {
        ...
        // Output refinement cannot change which tokens the inner parser can
        // start with, so recovery analysis sees straight through it.
...
///
/// The argument names a unit value implementing the runtime `OutputRejection`
/// trait for the refined parser's output type. Requiring a path keeps the same
/// tokens usable from both strict and recovered generation, where the trait is
/// implemented separately for each generated model's output type.
#[requires(true)]
#[ensures(true)]
