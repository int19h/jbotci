...
`bityzba-macros` is the proc-macro implementation crate for `bityzba`.
Workspace code and downstream users should depend on and import macros from
`bityzba`; this crate is an implementation detail.

# bityzba

`bityzba` is the project-local design-by-contract crate. It is an
