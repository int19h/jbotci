...
impl PredicateSignature {
    ...
    pub fn drop_place(&self, place: u32) -> Result<Self, ApplicationTypeError> {
        let place = PositiveInteger::from_u32(place);
...
mod tests {
    ...
    fn predicate_fill_cursor_and_drop_place_preserve_labels() {
        ...
        let dropped = signature.drop_place(2).unwrap();
        assert_eq!(
            dropped
                .row()
                .slots()
    ...
    fn computed_at_domains_must_be_disjoint_and_relation_exact() {
        ...
        let second = PredicateArgument::computed(
            place_type,
            vec![PlaceLabel::numbered(2), PlaceLabel::numbered(3)],
            referents,
        );
