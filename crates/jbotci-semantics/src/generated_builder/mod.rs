...
impl<'a, 'dict> GeneratedGraphBuilder<'a, 'dict> {
    ...
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_generated_assigned_pro_bridi_reference_formula(
        &mut self,
    ...
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_property_formula_for_assigned_pro_bridi_binding(
        &mut self,
        binding: &GeneratedAssignedProBridiBinding,
        parameter: SemanticObjectId,
        source: Option<crate::model::SemanticSource>,
    ...
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn wrap_formula_with_generated_assignment_scopes<'syntax>(
        &mut self,
        formula: SemanticObjectId,
    ...
    fn build_generated_quantified_respectively_distribution_formula(
        &mut self,
        formula: SemanticObjectId,
        variable: SemanticObjectId,
        quantity: Option<SemanticObjectId>,
    ...
    #[ensures(true)]
    fn exact_generated_integer_quantity_value(&self, quantity: SemanticObjectId) -> Option<usize> {
    ...
    fn generated_respectively_composite_argument_paired_with_variable(
        &self,
        formula: SemanticObjectId,
        variable: SemanticObjectId,
    ...
    fn generated_respectively_composite_members(
        &self,
        referent: SemanticObjectId,
    ...
    fn build_generated_parameter_with_source(
        &mut self,
        introduced_by: String,
        source: Option<crate::model::SemanticSource>,
        sort: SemanticSort,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent) || ret.is_err())]
    fn build_generated_respective_witness_referent(
        &mut self,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
        let id = self.next_referent_id();
    ...
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn wrap_formula_with_generated_argument_bundle_scope(
        &mut self,
        formula: SemanticObjectId,
    ...
    fn reserve_generated_argument_bundle_scope_ids(&mut self, scope_count: usize) {
    ...
    fn generated_argument_restrictions_for_scope_source_in_formula_context(
        &mut self,
        formula: SemanticObjectId,
    ...
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_tanru_formula_for_terms_with_head_eventuality_order_and_mode(
        &mut self,
        tanru: &TanruSelbriSyntax,
        terms: Vec<&TermSyntax>,
        first_visible_place: usize,
    ...
    #[requires(eventuality.is_none_or(|id| id.referent_sort().is_some_and(|sort| sort.is_subsort_of(SemanticSort::eventuality()))))]
    #[ensures(ret.as_ref().is_ok_and(|result| result.formula.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_assigned_pro_bridi_tanru_unit_formula_for_visible_arguments(
        &mut self,
    ...
    #[requires(eventuality.is_none_or(|id| id.referent_sort().is_some_and(|sort| sort.is_subsort_of(SemanticSort::eventuality()))))]
    #[ensures(ret.as_ref().is_ok_and(|result| result.formula.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_assigned_pro_bridi_tanru_unit_head_relation_formula_with_modal_terms(
        &mut self,
        unit: &AssignedProBridiTanruUnitSyntax,
        visible_arguments: BTreeMap<usize, ArgumentValue>,
        eventuality: Option<SemanticObjectId>,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|assignments| assignments.visible_arguments.keys().all(|place| *place > 0)) || ret.is_err())]
    fn build_term_assignments_for_terms_with_shared_tail_source<'syntax>(
        &mut self,
        terms: Vec<&'syntax TermSyntax>,
        first_visible_place: usize,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|assignments| assignments.visible_arguments.keys().all(|place| *place > 0)) || ret.is_err())]
    fn build_term_assignments_for_terms_without_shared_tail_source<'syntax>(
        &mut self,
        terms: Vec<&'syntax TermSyntax>,
        first_visible_place: usize,
    ) -> Result<GeneratedTermAssignments<'syntax>, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|assignments| assignments.visible_arguments.keys().all(|place| *place > 0)) || ret.is_err())]
    fn build_term_assignments_for_terms_with_shared_tail_source_core<'syntax>(
        &mut self,
        terms: Vec<&'syntax TermSyntax>,
        first_visible_place: usize,
        shared_tail_start: Option<usize>,
    ...
    fn build_term_assignments_for_terms_excluding_source<'syntax>(
        &mut self,
        terms: Vec<&'syntax TermSyntax>,
        first_visible_place: usize,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent) || ret.is_err())]
    fn build_property_abstraction_for_co_selbri(
        &mut self,
    ...
    #[requires(visible_arguments.keys().all(|place| *place > 0))]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent && id.referent_sort() == Some(SemanticSort::Relation)) || ret.is_err())]
    fn build_property_abstraction_for_co_selbri_with_visible_arguments(
        &mut self,
        selbri: &CoSelbriSyntax,
        mut visible_arguments: BTreeMap<usize, ArgumentValue>,
        source: Option<crate::model::SemanticSource>,
    ...
    #[requires(parameter.object_kind() == crate::model::SemanticObjectKind::Parameter)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_property_formula_for_co_selbri(
        &mut self,
        selbri: &CoSelbriSyntax,
        parameter: SemanticObjectId,
        source: Option<crate::model::SemanticSource>,
    ...
    #[requires(visible_arguments.keys().all(|place| *place > 0))]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_property_formula_for_co_selbri_with_visible_arguments(
        &mut self,
        selbri: &CoSelbriSyntax,
        visible_arguments: BTreeMap<usize, ArgumentValue>,
        source: Option<crate::model::SemanticSource>,
    ...
    #[requires(parameter.object_kind() == crate::model::SemanticObjectKind::Parameter)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_property_formula_for_selbri_with_context(
        &mut self,
        selbri: &SelbriSyntax,
        parameter: SemanticObjectId,
        source: Option<crate::model::SemanticSource>,
    ...
    #[requires(visible_arguments.keys().all(|place| *place > 0))]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_property_formula_for_connected_selbri_with_visible_arguments(
        &mut self,
        selbri: &ConnectedSelbriSyntax,
        visible_arguments: BTreeMap<usize, ArgumentValue>,
        source: Option<crate::model::SemanticSource>,
    ...
    #[requires(visible_arguments.keys().all(|place| *place > 0))]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_property_formula_for_tanru_selbri_with_visible_arguments(
        &mut self,
        tanru: &TanruSelbriSyntax,
        visible_arguments: BTreeMap<usize, ArgumentValue>,
        source: Option<crate::model::SemanticSource>,
    ...
    #[requires(visible_arguments.keys().all(|place| *place > 0))]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_property_formula_for_tanru_unit_with_visible_arguments(
        &mut self,
        unit: &TanruUnitSyntax,
        visible_arguments: BTreeMap<usize, ArgumentValue>,
        source: Option<crate::model::SemanticSource>,
    ...
    #[requires(visible_arguments.keys().all(|place| *place > 0))]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_connected_property_formula_for_tanru_unit_chain_with_visible_arguments(
        &mut self,
        unit: &TanruUnitSyntax,
        visible_arguments: BTreeMap<usize, ArgumentValue>,
        source: Option<crate::model::SemanticSource>,
    ...
    #[requires(visible_arguments.keys().all(|place| *place > 0))]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_connected_branch_property_formula_for_bo_or_linked_tanru_unit(
        &mut self,
        unit: &BoOrLinkedTanruUnitSyntax,
        visible_arguments: BTreeMap<usize, ArgumentValue>,
        eventuality: GeneratedPredicationEventuality,
    ...
    #[requires(visible_arguments.keys().all(|place| *place > 0))]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_property_formula_for_bo_or_linked_tanru_unit_with_visible_arguments(
        &mut self,
        unit: &BoOrLinkedTanruUnitSyntax,
        visible_arguments: BTreeMap<usize, ArgumentValue>,
        source: Option<crate::model::SemanticSource>,
    ...
    #[ensures(ret.as_ref().is_ok_and(|assignments| assignments.next_visible_place >= first_visible_place && assignments.visible_arguments.keys().all(|place| *place > 0)) || ret.is_err())]
    fn visible_arguments_adjusted_for_linkarg_assignments(
        visible_arguments: BTreeMap<usize, ArgumentValue>,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|argument| argument.value.is_some()) || ret.is_err())]
    fn build_tagged_or_elided_sumti_argument_with_visible_arguments(
        &mut self,
        sumti: &TaggedOrElidedSumtiSyntax,
    ...
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_property_atom_for_generated_relation_question(
        &mut self,
    ...
    fn build_restrictive_formula_for_generated_pro_bridi_frame(
        &mut self,
        cmavo: Cmavo,
        x1: SemanticObjectId,
    ...
    #[requires(formula.object_kind() == crate::model::SemanticObjectKind::Formula)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.referent_sort().is_some_and(|sort| sort.is_subsort_of(SemanticSort::eventuality()))) || ret.is_err())]
    fn eventuality_for_generated_formula(
        &self,
        formula: SemanticObjectId,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(right.object_kind() == crate::model::SemanticObjectKind::Formula)]
    #[ensures(true)]
    fn mark_generated_statement_whether_or_not_inert_operand(
        &mut self,
    ...
    #[requires(right.object_kind() == crate::model::SemanticObjectKind::Formula)]
    #[ensures(true)]
    fn mark_generated_bridi_tail_whether_or_not_inert_operand(
        &mut self,
    ...
    #[requires(right.object_kind() == crate::model::SemanticObjectKind::Formula)]
    #[ensures(true)]
    fn mark_generated_modal_forethought_whether_or_not_inert_operand(
        &mut self,
    ...
    fn insert_generated_termset_sumti_assignment<'syntax>(
        &mut self,
        visible_arguments: &mut BTreeMap<usize, ArgumentValue>,
        formula_scopes: &mut Vec<GeneratedArgumentQuantifierScope<'syntax>>,
        coequal_scope_groups: &mut Vec<GeneratedArgumentQuantifierBundleScope<'syntax>>,
    ...
    fn build_generated_ad_hoc_modal_argument_for_selbri<N: TreeNode>(
        &mut self,
        tense_modal: &N,
        selbri: &SelbriSyntax,
        argument: ArgumentValue,
    ...
    #[requires(true)]
    #[ensures(true)]
    fn modal_argument_modifiers_for_generated_tense_modal<N: TreeNode>(
        &self,
        tense_modal: &N,
    ...
    fn build_generated_jai_modal_argument(
        &mut self,
    ...
    fn build_generated_jai_modal_argument_for_referent(
        &mut self,
        unit: &JaiModalTanruUnitSyntax,
        referent: SemanticObjectId,
    ) -> Result<Option<ModalArgument>, SemanticsError> {
    ...
    fn generated_host_event_modal_elision(
        &mut self,
        eventuality: SemanticObjectId,
        modal_argument: &ModalArgument,
        place: usize,
    ...
    #[requires(true)]
    #[ensures(true)]
    fn record_generated_sticky_modal_argument_if_needed<N: TreeNode>(
        &mut self,
        tense_modal: &N,
        modal_argument: &ModalArgument,
    ) {
    ...
    #[requires(true)]
    #[ensures(true)]
    fn append_generated_sticky_modal_arguments(
        &mut self,
    ...
    #[requires(eventuality.is_none_or(|id| id.referent_sort().is_some_and(|sort| sort.is_subsort_of(SemanticSort::eventuality()))))]
    #[ensures(ret.as_ref().is_ok_and(|id| id.is_none_or(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula)) || ret.is_err())]
    fn build_resolved_generated_pro_bridi_formula_for_terms(
        &mut self,
        cmavo: Cmavo,
        terms: Vec<&TermSyntax>,
        first_visible_place: usize,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.referent_sort().is_some_and(|sort| sort.is_subsort_of(SemanticSort::eventuality()))) || ret.is_err())]
    fn build_generated_pro_bridi_eventuality_from_frame(
        &mut self,
    ...
    fn with_sumti_referent_cache_bypassed<T>(
        &mut self,
    ...
    fn with_pending_sumti_candidates_for_terms<T>(
        &mut self,
        terms: &[&TermSyntax],
        body: impl FnOnce(&mut Self) -> Result<T, SemanticsError>,
    ) -> Result<T, SemanticsError> {
    ...
    fn collect_pending_sumti_candidates_for_term(
        &self,
        term: &TermSyntax,
        candidates: &mut Vec<GeneratedPendingSumtiCandidate>,
    ) -> Result<(), SemanticsError> {
        match term {
    ...
    #[requires(true)]
    #[ensures(true)]
    fn collect_pending_sumti_candidates_for_simple_term(
        &self,
    ...
    #[requires(true)]
    #[ensures(true)]
    fn push_pending_sumti_candidate(
        &self,
        sumti: &SumtiSyntax,
        candidates: &mut Vec<GeneratedPendingSumtiCandidate>,
    ) {
        ...
        candidates.push(new!(GeneratedPendingSumtiCandidate {
            source_key,
            sumti: sumti.clone(),
        }));
    }
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.is_none_or(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent)) || ret.is_err())]
    fn build_pending_previous_sumti_referent<N: TreeNode>(
        &mut self,
        node: &N,
    ) -> Result<Option<SemanticObjectId>, SemanticsError> {
    ...
    fn build_generated_governed_termsets_for_terms<'syntax>(
        &mut self,
        terms: &[&'syntax TermSyntax],
    ...
    fn build_generated_governed_termset<'syntax>(
        &mut self,
        termset_index: usize,
    ...
    fn build_generated_letteral_sign_for_sumti(
        &mut self,
        sumti: &SumtiSyntax,
    ) -> Result<Option<SemanticObjectId>, SemanticsError> {
    ...
    fn build_generated_connective_fragment_sign<N: TreeNode>(
        &mut self,
        fragment: &N,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent && id.referent_sort() == Some(SemanticSort::Sign)) || ret.is_err())]
    fn build_generated_connective_sign<N: TreeNode>(
        &mut self,
        fragment: &N,
    ...
    fn build_generated_tense_modal_fragment_content<N: TreeNode>(
        &mut self,
        tense_modal: &N,
        sumti: &TaggedOrElidedSumtiSyntax,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn conjoin_generated_bare_jai_involvement_formula(
        &mut self,
    ...
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_generated_binary_constructed_relation_formula(
        &mut self,
        relation: &str,
        x1: SemanticObjectId,
        x2: SemanticObjectId,
    ...
    #[ensures(true)]
    fn generated_referent_is_abstraction_about_operand(
        &self,
        referent: SemanticObjectId,
        word: &str,
    ...
    fn recent_sumti_referent_before_node<N: TreeNode>(
        &self,
        node: &N,
        offset: usize,
    ) -> Option<SemanticObjectId> {
    ...
    #[requires(referent.object_kind() == crate::model::SemanticObjectKind::Referent)]
    #[ensures(true)]
    fn record_generated_letter_sumti_antecedent(
        &mut self,
        sumti: &SumtiSyntax,
        referent: SemanticObjectId,
    ) {
    ...
    fn attach_generated_relative_clauses_to_argument(
        &mut self,
        argument: ArgumentValue,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent) || ret.is_err())]
    fn build_plain_scoped_argument_variable_for_generated_sumti(
        &mut self,
        sumti: &SumtiSyntax,
    ) -> Result<SemanticObjectId, SemanticsError> {
        if let Some(key) = self.source_key_for_node(sumti)
    ...
    fn generated_requantified_da_source_for_sumti<'syntax>(
        &self,
        sumti: &'syntax SumtiSyntax,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_none_or(|binding| binding.variable.object_kind() == crate::model::SemanticObjectKind::Referent))]
    fn generated_requantified_da_source_for_sumti_bound<'syntax>(
        &self,
        sumti: &'syntax SumtiBoundSyntax,
        formula_scopes: &[GeneratedArgumentQuantifierScope<'syntax>],
    ) -> Option<GeneratedDaSeriesScopeBinding<'syntax>> {
    ...
    #[requires(!word.is_empty())]
    #[ensures(ret.as_ref().is_none_or(|binding| binding.variable.object_kind() == crate::model::SemanticObjectKind::Referent))]
    fn generated_requantified_da_source_for_word<'syntax>(
        &self,
        word: &str,
        formula_scopes: &[GeneratedArgumentQuantifierScope<'syntax>],
    ) -> Option<GeneratedDaSeriesScopeBinding<'syntax>> {
    ...
    #[requires(restrictions.iter().all(|restriction| restriction.object_kind() == crate::model::SemanticObjectKind::Formula))]
    #[ensures(true)]
    fn record_generated_quantified_da_series_binding(
        &mut self,
        scope: &GeneratedArgumentQuantifierScope<'_>,
    ...
    #[requires(variable.object_kind() == crate::model::SemanticObjectKind::Referent)]
    #[ensures(ret.as_ref().is_ok_and(|restrictions| restrictions.iter().all(|restriction| restriction.object_kind() == crate::model::SemanticObjectKind::Formula)) || ret.is_err())]
    fn lower_generated_da_series_scope_binding_restrictions(
        &mut self,
        binding: &GeneratedDaSeriesScopeBinding<'_>,
        variable: SemanticObjectId,
    ) -> Result<Vec<SemanticObjectId>, SemanticsError> {
    ...
    #[requires(variable.object_kind() == crate::model::SemanticObjectKind::Referent)]
    #[ensures(ret.as_ref().is_ok_and(|restrictions| restrictions.iter().all(|restriction| restriction.object_kind() == crate::model::SemanticObjectKind::Formula)) || ret.is_err())]
    fn lower_generated_argument_scope_source_restriction_nodes(
        &mut self,
        scope: &GeneratedArgumentQuantifierScope<'_>,
        variable: SemanticObjectId,
    ) -> Result<Vec<SemanticObjectId>, SemanticsError> {
    ...
    #[requires(variable.object_kind() == crate::model::SemanticObjectKind::Referent)]
    #[ensures(ret.as_ref().is_ok_and(|restrictions| restrictions.iter().all(|restriction| restriction.object_kind() == crate::model::SemanticObjectKind::Formula)) || ret.is_err())]
    fn lower_generated_argument_scope_node_relative_restrictions(
        &mut self,
        nodes: &[GeneratedArgumentQuantifierScopeNode<'_>],
        variable: SemanticObjectId,
    ) -> Result<Vec<SemanticObjectId>, SemanticsError> {
    ...
    fn clone_generated_restriction_formulas_for_variable(
        &mut self,
    ...
    #[ensures(ret.as_ref().is_ok_and(|formula| formula.is_none_or(|formula| formula.object_kind() == crate::model::SemanticObjectKind::Formula)) || ret.is_err())]
    fn clone_generated_formula_with_argument_replacements(
        &mut self,
        formula: SemanticObjectId,
    ...
    fn clone_generated_predication_with_argument_replacements(
        &mut self,
        predication: SemanticObjectId,
        replacements: &BTreeMap<SemanticObjectId, SemanticObjectId>,
    ) -> Result<Option<SemanticObjectId>, SemanticsError> {
    ...
    #[requires(variable.object_kind() == crate::model::SemanticObjectKind::Referent)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn clone_generated_formula_replacing_referent(
        &mut self,
        formula: SemanticObjectId,
    ...
    #[requires(variable.object_kind() == crate::model::SemanticObjectKind::Referent)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Predication) || ret.is_err())]
    fn clone_generated_predication_replacing_referent(
        &mut self,
        predication: SemanticObjectId,
        source_variable: SemanticObjectId,
        variable: SemanticObjectId,
    ...
    #[requires(eventuality.object_kind() == crate::model::SemanticObjectKind::Referent)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent) || ret.is_err())]
    fn clone_generated_eventuality_for_predication(
        &mut self,
        eventuality: SemanticObjectId,
    ) -> Result<SemanticObjectId, SemanticsError> {
        let object =
    ...
    fn clone_generated_argument_value_replacing_referent(
        &mut self,
        argument: &ArgumentValue,
        source_variable: SemanticObjectId,
        variable: SemanticObjectId,
    ...
    #[requires(referent.object_kind() == crate::model::SemanticObjectKind::Referent)]
    #[ensures(true)]
    fn generated_referent_is_elided_constant(&self, referent: SemanticObjectId) -> bool {
        self.objects.get(&referent).is_some_and(|object| {
    ...
    #[requires(referent.object_kind() == crate::model::SemanticObjectKind::Referent)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent) || ret.is_err())]
    fn clone_generated_referent_for_restriction(
        &mut self,
        referent: SemanticObjectId,
    ) -> Result<SemanticObjectId, SemanticsError> {
        let object = self
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent) || ret.is_err())]
    fn build_scoped_generated_pro_sumti_variable(
        &mut self,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.is_none_or(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent)) || ret.is_err())]
    fn build_generated_relative_sumti_referent(
        &mut self,
    ...
    #[requires(true)]
    #[ensures(true)]
    fn assigned_name_for_generated_relative_sumti(
        &self,
        sumti: &RelativeSumtiSyntax,
        clause: &SumtiAssociationRelativeClauseSyntax,
    ) -> Option<AssignedName> {
    ...
    #[requires(true)]
    #[ensures(true)]
    fn assigned_name_for_generated_tagged_or_elided_sumti(
        &self,
        sumti: &TaggedOrElidedSumtiSyntax,
        clause: &SumtiAssociationRelativeClauseSyntax,
    ) -> Option<AssignedName> {
    ...
    #[requires(true)]
    #[ensures(true)]
    fn assigned_name_for_generated_sumti(
        &self,
        sumti: &SumtiSyntax,
        clause: &SumtiAssociationRelativeClauseSyntax,
    ) -> Option<AssignedName> {
    ...
    #[requires(true)]
    #[ensures(true)]
    fn source_for_generated_assigned_name_clause(
        &self,
        clause: &SumtiAssociationRelativeClauseSyntax,
    ) -> Option<crate::model::SemanticSource> {
    ...
    #[requires(head.object_kind() == crate::model::SemanticObjectKind::Referent)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_generated_nonveridical_relative_formula_for_selbri(
        &mut self,
        selbri: &SelbriSyntax,
        head: SemanticObjectId,
        source: Option<crate::model::SemanticSource>,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent && id.referent_sort() == Some(SemanticSort::Relation)) || ret.is_err())]
    fn build_generated_property_abstraction_for_selbri_with_source(
        &mut self,
        selbri: &SelbriSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.is_none_or(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent))]
    fn resolved_generated_lerfu_sumti_referent(
        &self,
        sumti: &LerfuStringSumtiSyntax,
    ) -> Option<SemanticObjectId> {
    ...
    fn build_generated_quoted_text_group(
        &mut self,
        text: &TextSyntax,
        marker_free_modifiers: &[FreeModifierSyntax],
        source: Option<crate::model::SemanticSource>,
    ) -> Result<Option<SemanticObjectId>, SemanticsError> {
    ...
    fn build_generated_text_group_statement(
        &mut self,
    ...
    fn build_generated_text_group_statement_with_id(
        &mut self,
    ...
    fn build_generated_text_group_sequence(
        &mut self,
        text: &TextSyntax,
    ) -> Result<SemanticObjectId, SemanticsError> {
        let plan = generated_text_plan_from_text(text)?;
    ...
    fn ensure_generated_text_group_sequence_content(
        &mut self,
        item: SemanticObjectId,
        text: &TextSyntax,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent && id.referent_sort() == Some(SemanticSort::Sign)) || ret.is_err())]
    fn build_generated_math_expression_sign(
        &mut self,
        number: &NumberSumtiSyntax,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent && id.referent_sort() == Some(SemanticSort::Sign)) || ret.is_err())]
    fn build_generated_letteral_sign(
        &mut self,
        tokens: &[Token],
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    fn build_quantity_for_generated_mekso(
        &mut self,
    ...
    fn build_generated_math_expression(
        &mut self,
        expression: &MeksoSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    fn build_generated_math_expression_with_connected_operator_replacement(
        &mut self,
        expression: &MeksoSyntax,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|(id, _)| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_infix_mekso_with_connected_operator_replacement(
        &mut self,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|(id, _)| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_mekso_precedence_with_connected_operator_replacement(
        &mut self,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|(id, _)| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_mekso_base_with_connected_operator_replacement(
        &mut self,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|(id, _)| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_mekso_operand_with_connected_operator_replacement(
        &mut self,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|(id, _)| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_bound_or_simple_mekso_operand_with_connected_operator_replacement(
        &mut self,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|(id, _)| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_bound_mekso_operand_with_connected_operator_replacement(
        &mut self,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|(id, _)| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_simple_mekso_operand_with_connected_operator_replacement(
        &mut self,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_infix_mekso(
        &mut self,
        infix: &InfixMeksoSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_mekso_operand(
        &mut self,
        operand: &MeksoOperandSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_bound_or_simple_mekso_operand(
        &mut self,
        operand: &BoundOrSimpleMeksoOperandSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_bound_mekso_operand(
        &mut self,
        operand: &BoundMeksoOperandSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_simple_mekso_operand(
        &mut self,
        operand: &SimpleMeksoOperandSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_forethought_mekso_operand(
        &mut self,
        operand: &ForethoughtMeksoOperandSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_qualified_mekso_operand(
        &mut self,
        operand: &QualifiedMeksoOperandSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
        let id = self.build_generated_mekso_operand(&operand.inner_expression, source)?;
        self.set_math_scalar_negation(id, scalar_negation_for_token(&operand.nahe));
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_sumti_mekso_operand(
        &mut self,
        operand: &SumtiMeksoOperandSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    fn build_generated_sumti_operand_denotation(
        &mut self,
        sumti: &SumtiSyntax,
    ) -> Result<SemanticObjectId, SemanticsError> {
        let scope_source =
    ...
    #[requires(restrictions.iter().all(|restriction| restriction.object_kind() == crate::model::SemanticObjectKind::Formula))]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn generated_sumti_operand_body_formula(
        &mut self,
        variable: SemanticObjectId,
        restrictions: &[SemanticObjectId],
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_selbri_mekso_operand(
        &mut self,
        operand: &SelbriMeksoOperandSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
        let source = source.or_else(|| self.source_for_node(operand, "selbri-operand"));
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_array_mekso_operand(
        &mut self,
        operand: &ArrayMeksoOperandSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_forethought_call_mekso(
        &mut self,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_reverse_polish_mekso(
        &mut self,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_math_expression_for_number(
        &mut self,
        number: &NumberMeksoSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_math_expression_for_lerfu_string(
        &mut self,
        letter: &LerfuStringMeksoSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_math_operator_expression_for_operator(
        &mut self,
        operator: &MeksoOperatorSyntax,
        operands: Vec<SemanticObjectId>,
        source: Option<crate::model::SemanticSource>,
    ...
    fn generated_math_operator_question_parameter_for_operator(
        &mut self,
        operator: &MeksoOperatorSyntax,
    ) -> Result<Option<SemanticObjectId>, SemanticsError> {
    ...
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Parameter) || ret.is_err())]
    fn build_generated_math_operator_question_parameter_for_token(
        &mut self,
        token: &Token,
    ) -> Result<SemanticObjectId, SemanticsError> {
        let parameter = self.next_parameter_id();
    ...
    fn generated_math_operator_denotation_for_operator(
        &mut self,
        operator: &MeksoOperatorSyntax,
    ) -> Result<Option<SemanticObjectId>, SemanticsError> {
        match operator {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.is_none_or(|id| argument_object_kind_can_fill(id.object_kind()))) || ret.is_err())]
    fn generated_math_operator_denotation_for_bound_or_atom(
        &mut self,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.is_none_or(|id| argument_object_kind_can_fill(id.object_kind()))) || ret.is_err())]
    fn generated_math_operator_denotation_for_simple_operator(
        &mut self,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent && id.referent_sort() == Some(SemanticSort::Relation)) || ret.is_err())]
    fn build_generated_math_operator_property_abstraction_for_selbri(
        &mut self,
        selbri: &SelbriSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(operands.iter().all(|operand| operand.object_kind() == crate::model::SemanticObjectKind::MathExpression))]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_operand_connective_math_expression(
        &mut self,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::MathExpression) || ret.is_err())]
    fn build_generated_math_literal(
        &mut self,
        literal: MathLiteral,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(trailing.object_kind() == crate::model::SemanticObjectKind::Referent)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent) || ret.is_err())]
    fn build_connected_generated_forethought_sumti_referent<N: TreeNode>(
        &mut self,
        node: &N,
        source: SemanticObjectId,
        connective: &jbotci_syntax::generated_model::ModalForethoughtConnectiveSyntax,
    ...
    #[ensures(ret.as_ref().is_ok_and(|id| id.is_none_or(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent)) || ret.is_err())]
    fn generated_prenex_bound_pro_sumti(
        &mut self,
        pro_sumti: &ProSumtiSyntax,
    ) -> Result<Option<SemanticObjectId>, SemanticsError> {
    ...
    fn generated_implicit_existentials_for_active_prenex_bindings(
        &self,
    ...
    #[requires(matches!(variable.object_kind(), crate::model::SemanticObjectKind::Referent | crate::model::SemanticObjectKind::Parameter))]
    #[ensures(true)]
    fn generated_variable_has_active_prenex_binding(&self, variable: SemanticObjectId) -> bool {
    ...
    #[requires(pro_sumti.0.value.cmavo().is_some_and(|cmavo| matches!(cmavo, Cmavo::Da | Cmavo::De | Cmavo::Di)))]
    #[ensures(ret.is_none_or(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent))]
    fn generated_implicit_da_series_bound_pro_sumti(
        &self,
        pro_sumti: &ProSumtiSyntax,
    ) -> Option<SemanticObjectId> {
        self.implicit_da_series_bindings
    ...
    fn record_generated_implicit_existential_once(
        &mut self,
        variable: SemanticObjectId,
        source: Option<crate::model::SemanticSource>,
    ) {
    ...
    #[requires(restrictions.iter().all(|restriction| restriction.object_kind() == crate::model::SemanticObjectKind::Formula))]
    #[ensures(true)]
    fn add_generated_implicit_existential_restrictions(
        &mut self,
        variable: SemanticObjectId,
        restrictions: Vec<SemanticObjectId>,
    ) {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent) || ret.is_err())]
    fn build_generated_pro_sumti_fallback_referent(
        &mut self,
        pro_sumti: &ProSumtiSyntax,
    ...
    #[requires(!introduced_by.is_empty())]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Parameter) || ret.is_err())]
    fn build_generated_place_question_parameter(
        &mut self,
        introduced_by: String,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent) || ret.is_err())]
    fn build_gadri_description_referent_with_source<N: TreeNode>(
        &mut self,
        description_node: &N,
        description_head: &DescriptionHeadSyntax,
        tail: &DescriptionTailSyntax,
    ...
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent) || ret.is_err())]
    fn build_connected_abstraction_description_output(
        &mut self,
        source: Option<crate::model::SemanticSource>,
        cmavo: Option<Cmavo>,
        abstraction: &AbstractionTanruUnitSyntax,
    ...
    #[requires(!abstraction.abstractor_connections.is_empty())]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_connected_abstraction_description_body(
        &mut self,
        referent: SemanticObjectId,
        abstraction: &AbstractionTanruUnitSyntax,
    ) -> Result<SemanticObjectId, SemanticsError> {
        let mut current = self.build_single_abstraction_description_link_formula(
            referent,
            &abstraction_branch_with_marker(
                abstraction,
                abstraction.nu.clone(),
                abstraction.nai.clone(),
            ),
            abstraction,
        )?;
    ...
    #[requires(referent.object_kind() == crate::model::SemanticObjectKind::Referent)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_single_abstraction_description_link_formula(
        &mut self,
        referent: SemanticObjectId,
        abstraction: &AbstractionTanruUnitSyntax,
        source_node: &AbstractionTanruUnitSyntax,
    ) -> Result<SemanticObjectId, SemanticsError> {
        let output = self.build_abstraction_output(
            abstraction,
            self.source_for_node(source_node, "abstraction"),
        )?;
        let kind = abstraction_kind_for_nu(abstraction);
        let mut arguments = BTreeMap::new();
        ...
        self.insert(
            ...
            SemanticObject::predication(
                ...
                arguments,
                PredicationMode::Restrictive,
                self.source_for_node(source_node, "abstraction-description"),
                Vec::new(),
            ),
        )?;
        let formula = self.next_formula_id();
        self.insert(
            ...
            SemanticObject::atom_formula(
                predication,
                self.source_for_node(source_node, "restrictive-formula"),
                Vec::new(),
            ),
        )?;
        Ok(formula)
    ...
    #[requires(right.object_kind() == crate::model::SemanticObjectKind::Formula)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_binary_formula_for_generated_abstraction_connective(
        &mut self,
        connective: &StatementConnectiveSyntax,
        left: SemanticObjectId,
        right: SemanticObjectId,
    ...
    #[requires(referent.object_kind() == crate::model::SemanticObjectKind::Referent)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_generated_selbri_name_description_formula<N: TreeNode>(
        &mut self,
        description_node: &N,
        selbri: &SelbriSyntax,
        referent: SemanticObjectId,
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent && id.referent_sort() == Some(SemanticSort::Sign)) || ret.is_err())]
    fn build_generated_selbri_name_sign(
        &mut self,
        selbri: &SelbriSyntax,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent && id.referent_sort() == Some(SemanticSort::Relation)) || ret.is_err())]
    fn build_description_property_abstraction_for_selbri_with_source(
        &mut self,
        selbri: &SelbriSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(referent.object_kind() == crate::model::SemanticObjectKind::Referent)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Formula) || ret.is_err())]
    fn build_restrictive_generated_jai_modal_conversion_formula(
        &mut self,
        selbri: &SelbriSyntax,
        unit: &TanruUnitSyntax,
        jai_unit: &JaiModalTanruUnitSyntax,
        referent: SemanticObjectId,
    ) -> Result<SemanticObjectId, SemanticsError> {
        let (atom, linkargs) = generated_linked_tanru_unit_parts(unit)?;
        let relation = semantic_relation_label(relation_label_from_tanru_unit_atom_base(
    ...
    fn connected_quantifier_quantity_scope_for_generated_quantifier(
        &mut self,
        quantifier: &QuantifierSyntax,
        locus: &str,
    ...
    #[requires(!locus.is_empty())]
    #[ensures(ret.as_ref().is_ok_and(|connection| connection.as_ref().is_none_or(|connection| connection.left_quantity.object_kind() == crate::model::SemanticObjectKind::Quantity && connection.right_quantity.object_kind() == crate::model::SemanticObjectKind::Quantity)) || ret.is_err())]
    fn connected_quantifier_quantity_scope_for_generated_mekso(
        &mut self,
        expression: &MeksoSyntax,
        locus: &str,
    ) -> Result<Option<GeneratedConnectedQuantifierQuantityScope>, SemanticsError> {
    ...
    fn build_generated_forethought_connected_quantifier_quantity_scope(
        &mut self,
        operand: &ForethoughtMeksoOperandSyntax,
        locus: &str,
        source: Option<crate::model::SemanticSource>,
    ...
    #[requires(!locus.is_empty())]
    #[ensures(ret.as_ref().is_ok_and(|connection| connection.left_quantity.object_kind() == crate::model::SemanticObjectKind::Quantity && connection.right_quantity.object_kind() == crate::model::SemanticObjectKind::Quantity) || ret.is_err())]
    fn build_generated_afterthought_connected_quantifier_quantity_scope(
        &mut self,
        left_operand: &BoundOrSimpleMeksoOperandSyntax,
        connective: &jbotci_syntax::generated_model::OperandConnectiveSyntax,
        right_operand: &BoundOrSimpleMeksoOperandSyntax,
        locus: &str,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<GeneratedConnectedQuantifierQuantityScope, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Quantity) || ret.is_err())]
    fn build_quantity_for_generated_bound_or_simple_mekso_operand(
        &mut self,
        operand: &BoundOrSimpleMeksoOperandSyntax,
        source: Option<crate::model::SemanticSource>,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(!construct.is_empty())]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Quantity) || ret.is_err())]
    fn build_quantity_for_generated_node<N: TreeNode>(
        &mut self,
        node: &N,
        construct: &str,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(!text.is_empty())]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Quantity) || ret.is_err())]
    fn build_quantity_for_generated_tokens(
        &mut self,
        words: &[Token],
        text: &str,
        source: Option<crate::model::SemanticSource>,
    ...
    #[requires(!text.is_empty())]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Quantity) || ret.is_err())]
    fn build_recurrence_quantity_for_generated_integer(
        &mut self,
        text: &str,
        value: i64,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Quantity) || ret.is_err())]
    fn build_recurrence_quantity_for_generated_value(
        &mut self,
        value: QuantityValue,
    ) -> Result<SemanticObjectId, SemanticsError> {
        self.build_recurrence_quantity_for_generated_value_with_form(
    ...
    #[requires(true)]
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Quantity) || ret.is_err())]
    fn build_recurrence_quantity_for_generated_value_with_form(
        &mut self,
        form: QuantityForm,
        value: QuantityValue,
    ) -> Result<SemanticObjectId, SemanticsError> {
    ...
    fn build_abstraction_output(
        ...
        let body = match self
            .build_generated_subbridi_formula(&abstraction.subbridi, abstraction_body_mode(kind))
        {
            Ok(result) => result,
            Err(error) => {
    ...
    #[ensures(ret.as_ref().is_ok_and(|id| id.object_kind() == crate::model::SemanticObjectKind::Referent) || ret.is_err())]
    fn build_generated_abstraction_about_referent(
        &mut self,
        word: &str,
        operand: SemanticObjectId,
        source: Option<crate::model::SemanticSource>,
