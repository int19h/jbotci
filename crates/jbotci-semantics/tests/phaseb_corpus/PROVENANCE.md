...
- Amendment 5 (jbotci#620 round-1 review B3), oracle commit
  ...
  earlier 38 documents change (they carry no `placeQuestions`).
- Corpus: 33 `battery-docs/b*.json`/`nd*.json` + 4
  ...
  `0263b93`), each whose `.frozen.json` is generated by *this* jbotci build's
...
- Aggregate fixture hashes over all 39 documents (pinned in
