...
fn parse_typed_graph(datum: &Datum) -> Result<TypedGraph, V0ParseError> {
    ...
    require_len(items, 3, "TypedGraph")?;
...
/// Stable fallback ids are ASCII, dotted names rooted at `smusni`.
#[requires(true)]
#[ensures(true)]
  ...
  (Words (Word klama "x1 goes")))"#;
    ...
    fn fallback_grammar_enforces_identity_order_and_reason_domain() {
        ...
        assert!(parse_v0_document(
            r#"(Smusni 0 (TypedGraph "SemanticGraph" (Object %1 "Root" (Field "self" (Ref %1)))))"#
        )
        .is_ok());
        assert!(
            parse_v0_document(r#"(Smusni 0 (TypedGraph "SemanticGraph" (Object %2 "Root")))"#)
                .is_err()
        );
        assert!(
        ...
        for malformed in [
            ...
            r#"(Fallback Content "smusni.test" (Ref %1))"#,
            r#"(Fallback Content "smusni.test" (Object %1 "Root" (Field "x" (Object %1 "Again"))))"#,
            r#"(Fallback Content "smusni.test" (UnknownRaw))"#,
            r#"(Fallback Content "smusni.test" (RawNull extra))"#,
            r#"(Fallback Content "smusni.test" "raw")"#,
        ] {
            assert!(
