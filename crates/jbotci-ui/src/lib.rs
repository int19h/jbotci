...
use jbotci_web_core::{
    ...
    GimfihiCandidate, GimfihiOutput, GimfihiPreset, GimfihiPresetOption, GimfihiScorer,
    GimfihiWebResult, GimfihiWebSource, GimfihiWebState, GismuShape, MANIFEST_ASSET_PATH, PageMeta,
    RafsiAvailabilityData, RafsiCandidate, RafsiClaimKind, ReferenceLabel, ReferenceMarker,
    ReferenceMarkerRole, ReferenceTooltip, ReferenceTooltipInline, ReferenceTooltipInlineData,
    ReferenceTooltipRow, VLACKU_WEB_DEFAULT_COUNT, VLACKU_WEB_MAX_COUNT, VlackuCompositionPiece,
...
// The worker client and workers add app-bundle URL versioning when loading these
// modules. The server/service worker must not serve either the literal paths or
// wasm-bindgen snippet copies of the helper modules as immutable.
...
const DEFAULT_GENTUFA_TEXT: &str = jbotci_web_core::DEFAULT_GENTUFA_TEXT;
