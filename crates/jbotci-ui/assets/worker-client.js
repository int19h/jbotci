export function createWorkerClient(options) {
  ...
  const defaultWorkerUrl = options.defaultWorkerUrl;
  const defaultMinIdleWorkers = options.minIdleWorkers ?? 1;
  const defaultMaxIdleWorkers = options.maxIdleWorkers ?? 3;
  ...
  function workerUrl() {
    return configuredWorkerUrl ?? defaultWorkerUrl();
  }
  ...
  function appModuleUrl() {
    ...
    if (typeof url !== "string" || url.length === 0) {
      throw new Error(`${label} app module URL is not configured`);
    }
  ...
  function currentWorkerContext() {
    ...
    const keyParts = [
      workerUrlHref,
      mainModuleUrl,
      options.contextKey?.(config) ?? null,
    ];
    return {
      workerUrlHref,
      mainModuleUrl,
      ...
      minIdleWorkers: config.minIdleWorkers ?? defaultMinIdleWorkers,
      maxIdleWorkers: config.maxIdleWorkers ?? defaultMaxIdleWorkers,
    };
  }
  ...
  function terminateWorkerEntry(entry, reason = `${label} worker terminated`) {
  ...
  function handleWorkerMessage(entry, event) {
    ...
    if (message.kind === "ready") {
      if (message.ok === false) {
        const error = message.error || `${label} worker initialization failed`;
  ...
  function workerErrorMessage(event) {
    ...
    return event.message
      ? `${label} worker failed${location}: ${event.message}`
      : `${label} worker failed${location}`;
  }
  ...
  function createWorkerEntry(context) {
    ...
    worker.onmessageerror = () => {
      failActiveWorker(entry, `${label} worker returned an unreadable message`);
    };
    try {
      worker.postMessage(options.warmMessage(context));
    } catch (error) {
  ...
  function pruneSpareWorkers(key) {
    ...
    while (idle.length + warming.length > context.maxIdleWorkers) {
  ...
  function ensureWarmSpare() {
    ...
    while (spareWorkersForContext(context.key).length < context.minIdleWorkers) {
      try {
  ...
  function configureWorker(workerUrlString) {
    if (typeof workerUrlString !== "string" || workerUrlString.length === 0) {
      throw new Error(`${label} worker URL is empty`);
    }
    ...
    if (previousWorkerUrl !== nextWorkerUrl.href) {
      terminateAllWorkers(`${label} worker URL changed`);
    }
    configuredWorkerUrl = nextWorkerUrl;
    ensureWarmSpare();
  }

  function configureAppModule(appModuleUrlString) {
    if (typeof appModuleUrlString !== "string" || appModuleUrlString.length === 0) {
      throw new Error(`${label} app module URL is empty`);
    }
    ...
    if (previousAppModuleUrl !== null && previousAppModuleUrl !== nextAppModuleUrl) {
      terminateAllWorkers(`${label} app module URL changed`);
    }
  ...
  function cancel(channel) {
    ...
    for (const id of ids) {
      ...
      pending.delete(id);
      removeChannelRequest(request.channel, id);
      terminateWorkerEntry(request.workerEntry, `${label} request cancelled`);
      request.reject(`${label} request cancelled`);
    }
    ensureWarmSpare();
  }

  function hasPending(channel) {
    return (channelRequests.get(channel)?.size ?? 0) > 0;
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      workerEntry.readyPromise
        .then(() => {
          ...
          if (!current || current.workerEntry !== workerEntry) {
            return;
          }
          try {
            workerEntry.worker.postMessage(options.requestMessage({
              id,
              payload,
              workerEntry,
            }));
...
export function jbotciWorkerClientAssetPin() {}
