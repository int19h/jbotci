...
## Re-application at the epoch-9 base (2026-09-01)

This branch was frozen at `284147baa4`, whose base was `0d791fd35c`. Epoch 9 replays it onto the
merged epoch-8 base `9ec321d530`, and the two commits behave differently there:

* `f408cff986`, the code change, applies **cleanly**. Nothing in epochs 6b-8 touched the
  productions it names, so C1-C6 above hold verbatim at the epoch base.
* `284147baa4`, the expectation re-baseline, **conflicts in exactly three files, two hunks each**
  — `tests/fixtures/adhoc/syntax/selbri/issue-828-elided-ku-zantufa.toml`,
  `.../issue-828-explicit-ku-zantufa.toml` and `tests/fixtures/corpus/alis/full-alice.toml`.
  Every conflicting hunk is expectation payload (`raw`, `json`, `diagnostics`); no macro, runtime,
  DSL or source hunk conflicts at all.

The cause is C7's own footprint meeting epoch 8. Epoch 8 re-owned the relative-clause machinery
that those same three payloads render, after this branch froze, so the frozen payloads and the
epoch base's payloads differ in BOTH the epoch-8 relative re-owning and the #634 quantifier
re-typing. Carrying the frozen payloads across would have re-introduced pre-epoch-8 relative
ownership into three fixtures.

**C7's method was therefore re-applied rather than its results carried.** Every conflicted hunk
was resolved to the epoch-base side, and the four affected fixtures — the three above plus
`tests/fixtures/adhoc/syntax/tags/issue-833-stag-position-zantufa-rejected.toml`, which
auto-merged but whose payload is regenerated for the same reason — were regenerated with
`fixture-rewrite --path` on top of the applied code change. The regeneration was then validated
mechanically, not trusted, by the same kind of script C7 used
(`/build/jbotci/scratch/epoch09-impl/validate-cb.py`): it rewrites the OLD epoch-base expectation
with the two declared #634 shapes and requires the result to equal the regenerated expectation
structurally, so an ownership change cannot be laundered as a re-typing.
...
| `corpus/alis/full-alice` | 350 | 1 (`ForethoughtCallMekso`) | 0 (the fixture pins no diagnostics) |
| `adhoc/syntax/tags/issue-833-stag-position-zantufa-rejected` | 0 | 0 | 0 (rejection-diagnostic reclassification) |

352 re-typed positions and one surviving genuine raw mex, with **zero residue** — which is
exactly the census the epoch-9 pre-C-a base ledger measured at `9ec321d530` before any of this
landed, and exactly the 350/1 split C7 recorded at `0d791fd35c`. The `#833` fixture stays
rejected at the same offset with the same `status = "failure"`; only its winning diagnostic code
moves, for the reason C7 already gives.
