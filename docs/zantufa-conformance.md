...
| `bridi_tail_3 <- KE bridi_tail KEhE? tail_terms` | `ZantufaTerms` | `feature-required` | Accepted under `ZantufaTerms` as `ZantufaGroupedBridiTail`; gated because it conflicts with baseline `KE` tanru grouping |
