...
### Round 3: fixture-worker memory regression

| Surface or gate | Disposition and evidence |
|---|---|
| Full-Alice syntax worker peak RSS | The epoch initially doubled the peak of one isolated release fixture worker: base `9fafb66d4a` used 9,381,188 KiB, while round-2 `d0fddfe933` used 18,175,932 KiB on the identical `corpus/alis/full-alice.toml`, one-job, syntax-only command. Samples during the latter parse rose through 13.37, 15.80, 16.25, 16.69, 17.16, 17.62, and 18.03 GiB before plateauing. This rules out aggregate worker concurrency and retained cross-fixture output. Logs: `/build/jbotci/logs/epoch05-round3-base-full-alice-syntax-time.log` and `/build/jbotci/logs/epoch05-round3-current-full-alice-syntax-time.log`. |
| Dominant allocation site | A DHAT run over a representative 10,159-byte Alice slice under the Zantufa dialect measured 948,814,063 bytes live at the global heap maximum. Allocation stacks containing `SyntaxParseError::with_active_contexts` accounted for 768,637,576 bytes (81.0%): each newly memoized strict or recovery failure copied the complete active syntax-context path. The parameterized selbri memo scopes increased the number of legitimate failure entries, exposing this pre-existing quadratic snapshot representation; parser-graph construction, raw output trees, and recovery snapshots were not dominant. Profile summaries: `/build/jbotci/logs/epoch05-round3-dhat-alice01-zantufa.log`, `/build/jbotci/logs/epoch05-round3-dhat-top-pps.json`, and `/build/jbotci/logs/epoch05-round3-dhat-top-stacks.log`. |
| Persistent active-path snapshots | `SyntaxParseError` now retains persistent `Rc`-shared context and rule stacks. `ParserState` maintains these alongside its existing active-path vectors, including push, pop, and checkpoint rewind, so memo keys, parser selection, diagnostic ordering, and recovery behavior are unchanged. Exporting the final rule path materializes one ordered vector; intermediate failure enrichment is constant-size. The identical Full-Alice worker now peaks at 9,696,884 KiB: 46.6% below round 2 and only 3.4% above base, restoring substantial headroom beneath the 16 GiB runner ceiling. The fixture remains exact-pass with unchanged expectations. Log: `/build/jbotci/logs/epoch05-round3-fixed-full-alice-syntax-time.log`. |
| Struct invariant audit | `SharedStack` has the concrete non-empty/head consistency invariant. `SharedStackNode` admits every typed value plus optional parent combination, and `SharedStackIter` admits both an active borrowed node and exhausted `None`; their `#[invariant(true)]` markers are individually dispositioned in the workspace audit allowlist. |
| Round-3 verification | The release workspace suite and the single release all-targets expensive-contract suite are green. The complete all-profile run is unchanged at 26,413 fixtures / 73,703 passed / 515 xfailed / zero failed, with a whole-run peak of 5,738,732 KiB; the frozen selbri syntax facet is 111/111. Formatting, the debug `jbotci` build, and the debug Dioxus web build are also green. Logs: `/build/jbotci/logs/epoch05-round3-cargo-test-release-final.log`, `/build/jbotci/logs/epoch05-round3-full-fixture-release.log`, `/build/jbotci/logs/epoch05-round3-expensive-contracts-all-targets.log`, `/build/jbotci/logs/epoch05-round3-tagged-syntax.log`, `/build/jbotci/logs/epoch05-round3-debug-jbotci-build.log`, and `/build/jbotci/logs/epoch05-round3-debug-dx-build.log`. |
