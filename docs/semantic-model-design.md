...
*Synthesized reference, retained as design history. Its worked companion is
[`semantic-model-cll-encodings.md`](semantic-model-cll-encodings.md) (CLL
chapters 5–19 against this model).*

> **Status (jbotci#869).** This document describes the object model behind the
> `jbotci tersmu` implementation that was retired from this repository. Its
> JSON format specification (`semantic-model-spec.md`) went with that
...
The notation above is conceptual.  The public JSON shape of the retired
implementation emitted every denotable value as `type:"referent"`: events and states are referents with sort paths such as
...
for meaning and place count, so the graph emits no decomposition metadata. It
does not derive place identifications, dropped or retained places, participant
mappings, or implicit abstractions from the spelling or from free-form
definition text.

For a nonce lujvo absent from the dictionary, morphology may provide a complete
rafsi decomposition. When every rafsi resolves to a source word, `REL` records
only the resolved full `sourceWords` and the typed rafsi `expansion`, including
context-sensitive `rafsiBindings` when present. It makes no `placeStructure`
claim. The predication retains the ordinary unknown-place-structure warning and
only the places required by explicit assignments. This mechanical annotation
never adds asserted formula children.
