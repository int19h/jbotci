...
- **Quotation, two registers.** **Structured** `lu…li'u` → a **nested `UTT`** (force=mentioned, fully parsed content, with its **own speaker/audience roles**, and **referents reachable** from outside by shared id). **Opaque** `lo'u…le'u` / `zo` / `zoi` → a **`SGN kind=quotation`** (token sequence, no internal structure, **no reachable referents**). `zo` = single-word token; `zoi .X. … .X.` = foreign opaque text (the only way to quote non-Lojban / rafsi); `la'o .X. … .X.` = a **name from foreign text** (= `la me zoi`). **`la'e`/`lu'e`** = the reference↔referent shifts (`lae[]`/`lue[]`): the triad `zo .bab.` (word) / `la .bab.` (named thing) / `la'e zo .bab.` (the word's referent).
