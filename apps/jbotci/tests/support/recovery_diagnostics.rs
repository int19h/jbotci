...
const SYNTAX_EXPECTED_LABEL: &str = "expected: free modifier, space interval, sumti association phrase, time interval, interval, space tense, time tense, sumti relative phrase, termset connection continuation, termset connective, place tag, tag, paragraph statement, prenex, or paragraph";
