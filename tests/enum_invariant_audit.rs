...
const ALLOWED_PLACEHOLDERS: &[(&str, &str)] = &[
    ...
    (
        "crates/jbotci-cll/src/model.rs:CllDivision::Appendix",
        "an appendix is designated by its title alone, so the variant deliberately carries no number",
    ),
    (
        "crates/jbotci-cll/src/model.rs:CllDivision::Chapter",
        "the chapter number is a NonZeroU16, so the variant data already expresses its positivity",
    ),
    (
